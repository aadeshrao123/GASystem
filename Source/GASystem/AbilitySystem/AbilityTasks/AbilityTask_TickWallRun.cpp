...
void UAbilityTask_TickWallRun::Activate()
{
	...
	OnWallSideDetermined.Broadcast(IsWallOnLeft(OnWallHit));
...
void UAbilityTask_TickWallRun::TickTask(float DeltaTime)
{
	...
	*/
	...
	const FRotator DirectRotator = IsWallOnLeft(OnWallHit) ? FRotator(0, -90, 0) : FRotator(0, 90, 0);

	const FVector WallRunDirection = DirectRotator.RotateVector(OnWallHit.ImpactNormal);
...
bool UAbilityTask_TickWallRun::IsWallOnLeft(const FHitResult& InWallHit) const
{
	return FVector::DotProduct(CharacterOwner->GetActorRightVector(), InWallHit.ImpactPoint) > 0.f;
}
